...
export async function commitOrder() {
  ...
  try {
    ...
    if (!res.ok) throw new Error(data.message || 'Add failed');
        ...
        cartModal.hide();
        showOrderModal(data.username, data.my_order);
        refreshCart();
    }
  }
...
// Helpers
function showOrderModal(username, my_order) {
  document.getElementById('orderUsername').textContent = username;
  document.getElementById('orderDate').textContent = my_order.date;
  document.getElementById('orderAddress').textContent = my_order.address || 'N/A';
  document.getElementById('orderPayment').textContent = my_order.payment || 'N/A';
